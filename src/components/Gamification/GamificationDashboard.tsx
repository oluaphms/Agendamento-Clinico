...
export const GamificationDashboard: React.FC<GamificationDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'achievements' | 'leaderboard' | 'events'
  >('overview');
  ...
  const categoryStats = [
    {
      name: 'Agendamentos',
      icon: '📅',
      unlocked: getAchievementsByCategory('agendamentos').filter(
        a => a.isUnlocked
      ).length,
    ...
    },
    {
      name: 'Pacientes',
      icon: '👤',
      unlocked: getAchievementsByCategory('pacientes').filter(a => a.isUnlocked)
        .length,
    ...
    },
    {
      name: 'Profissionais',
      icon: '👨‍⚕️',
      unlocked: getAchievementsByCategory('profissionais').filter(
        a => a.isUnlocked
      ).length,
    ...
    },
    {
      name: 'Sistema',
      icon: '⚙️',
      unlocked: getAchievementsByCategory('sistema').filter(a => a.isUnlocked)
        .length,
    ...
    },
    {
      name: 'Consecutivo',
      icon: '🔥',
      unlocked: getAchievementsByCategory('consecutivo').filter(
        a => a.isUnlocked
      ).length,
          ...
          <div className='flex items-center space-x-3'>
            ...
            <div>
              ...
              <p className='text-sm text-gray-600'>
                Sistema de conquistas e engajamento
              </p>
            </div>
          </div>
            ...
            <div className='bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6'>
              <div className='flex items-center justify-between mb-3'>
                <h3 className='text-lg font-medium text-gray-900'>
                  Progresso para Nível {userStats.level + 1}
                </h3>
                  ...
                  >
                    <div className='flex items-center justify-between'>
                      ...
                      <div className='text-right'>
                        <div className='text-lg font-bold text-gray-900'>
                          {Math.round(
                            (category.unlocked / category.total) * 100
                          )}
                          %
                        </div>
                      </div>
                    ...
                    <div className='mt-3 w-full bg-gray-200 rounded-full h-2'>
                      <div
                        ...
                        style={{
                          width: `${(category.unlocked / category.total) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                ...
                recentEvents.map(event => (
                  ...
                  >
                    ...
                    <div className='flex-1'>
                      <div className='text-sm font-medium text-gray-900'>
                        {event.type === 'achievement_unlocked' &&
                          `Conquista desbloqueada: ${(event.data as any).achievementName}`}
                        {event.type === 'level_up' &&
                          `Nível ${(event.data as any).newLevel} alcançado!`}
                        {event.type === 'points_earned' &&
                          `+${(event.data as any).points} pontos: ${(event.data as any).reason}`}
                      </div>
