...
export const BackupManager: React.FC = () => {
  ...
  const handleRestoreBackup = async (backupId: string) => {
    if (window.confirm('Tem certeza que deseja restaurar este backup? Esta ação irá sobrescrever os dados atuais.')) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Gerenciador de Backup</h2>
          <p className="text-gray-600 dark:text-gray-400">Gerencie backups automáticos e manuais do sistema</p>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold">{formatFileSize(status.totalSize)}</p>
              </div>
              <HardDrive className="h-8 w-8 text-gray-500" />
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold">
                  {status.lastBackup ? getBackupAge(status.lastBackup) : 'Nunca'}
                </p>
              </div>
                ...
                {config.tables.map((table) => (
                  <label key={table} className="flex items-center">
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          updateConfig({ tables: [...config.tables, table] });
                        } else {
                          updateConfig({ tables: config.tables.filter(t => t !== table) });
                        }
                      }}
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <p>Registros: {backup.metadata.totalRecords}</p>
                      <p>Usuários: {backup.metadata.userCount}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button
