...
import { Container, Grid, GridItem, Flex } from '@/components/UI';
import { useForm } from '@/hooks';
import { TemplateConfig, TemplateSection, TemplateField, templateManager } from '@/lib/templates';
import { Plus, Trash2, Edit, Save, Eye, Download } from 'lucide-react';
...
const TemplateBuilder: React.FC<TemplateBuilderProps> = ({
  ...
  const updateField = useCallback((sectionId: string, fieldId: string, updates: Partial<TemplateField>) => {
    updateSection(sectionId, {
      fields: sections
        ...
        .map(field => field.id === fieldId ? { ...field, ...updates } : field) || [],
    });
  }, [sections, updateSection]);
  ...
  const renderFieldEditor = (section: TemplateSection, field: TemplateField) => (
      ...
      <CardBody>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              value={field.type}
              onChange={(e) => updateField(section.id, field.id, { type: e.target.value as TemplateField['type'] })}
