...
export interface UsePermissionsReturn {
  ...
  // Ações de permissões
  createPermission: (permission: Omit<Permission, 'id' | 'created_at' | 'updated_at'>) => Promise<void>;
  ...
  checkPermission: (resource: string, action: string) => Promise<PermissionCheck>;
  checkMultiplePermissions: (permissions: Array<{ resource: string; action: string }>) => Promise<Record<string, PermissionCheck>>;
...
export const usePermissions = (): UsePermissionsReturn => {
  ...
  const createPermission = useCallback(async (permission: Omit<Permission, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      setError(null);
  ...
  const createRole = useCallback(async (role: Omit<Role, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      setError(null);
  ...
  const removeRoleFromUser = useCallback(async (userId: string, roleId: string) => {
    try {
      ...
      setUserRoles(prev => prev.filter(ur => !(ur.user_id === userId && ur.role_id === roleId)));
    } catch (error) {
  ...
  const checkPermission = useCallback(async (resource: string, action: string): Promise<PermissionCheck> => {
    if (!user?.id) {
  ...
  const checkMultiplePermissions = useCallback(async (permissions: Array<{ resource: string; action: string }>): Promise<Record<string, PermissionCheck>> => {
    if (!user?.id) {
      return permissions.reduce((acc, permission) => {
        const key = `${permission.resource}:${permission.action}`;
        acc[key] = { hasPermission: false, reason: 'Usuário não autenticado' };
        return acc;
