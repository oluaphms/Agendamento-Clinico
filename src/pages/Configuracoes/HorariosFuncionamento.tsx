...
const HorariosFuncionamento: React.FC = () => {
  ...
  const [configuracoesEspeciais, setConfiguracoesEspeciais] = useState<
    ConfiguracaoEspecial[]
  >([]);
  ...
  // @ts-ignore
  const [configuracaoEspecialSelecionada, setConfiguracaoEspecialSelecionada] =
    useState<ConfiguracaoEspecial | null>(null);
  // @ts-ignore
  const [editando, setEditando] = useState(false);
  // @ts-ignore
  const [formData, setFormData] = useState<Partial<HorarioFuncionamento>>({});
  // @ts-ignore
  const [formDataEspecial, setFormDataEspecial] = useState<
    Partial<ConfiguracaoEspecial>
  >({});
  ...
  const loadDados = async () => {
    setLoading(true);
    try {
      await Promise.all([loadHorarios(), loadConfiguracoesEspeciais()]);
    } catch (error) {
  ...
  // const salvarHorario = async () => { // Comentado - não utilizado
  ...
  // const salvarConfiguracaoEspecial = async () => { // Comentado - não utilizado
  ...
  const handleExcluirEspecial = async (id: string) => {
    if (
      !window.confirm(
        'Tem certeza que deseja excluir esta configuração especial?'
      )
    ) {
      return;
    }
  ...
  const horariosFiltrados = horarios.filter(horario => {
    const matchesDiaSemana =
      !filtros.dia_semana ||
      horario.dia_semana.toString() === filtros.dia_semana;
  ...
  const configuracoesEspeciaisFiltradas = configuracoesEspeciais.filter(
    config => {
  ...
  const configuracoesEspeciaisAtivas = configuracoesEspeciais.filter(
    c => c.ativo
  ).length;
  ...
  return (
    <div className='min-h-screen bg-gray-50 dark:bg-gray-900 p-6'>
      <Helmet>
        ...
        <meta
          name='description'
          content='Configuração de horários de funcionamento da clínica'
        />
      </Helmet>
          ...
          <CardContent className='p-6'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-4 gap-4 mb-6'>
              ...
              <select
                value={filtros.turno}
                onChange={e =>
                  setFiltros({ ...filtros, turno: e.target.value })
                }
                className='px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm'
              >
              ...
              {horariosFiltrados.map(horario => (
                ...
                >
                  <div className='flex items-center space-x-4'>
                    ...
                    <div>
                      <h4 className='text-sm font-medium text-gray-900 dark:text-white'>
                        {getDiaSemana(horario.dia_semana)} -{' '}
                        {horario.turno.charAt(0).toUpperCase() +
                          horario.turno.slice(1)}
                      </h4>
                  ...
                  <div className='flex items-center space-x-2'>
                    <span
                      className={`px-2 py-1 text-xs rounded-full ${getTurnoColor(horario.turno)}`}
                    >
                      {horario.turno.charAt(0).toUpperCase() +
                        horario.turno.slice(1)}
                    </span>
          ...
          <CardContent className='p-6'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-4 gap-4 mb-6'>
              ...
              >
                ...
                <option value='atendimento_especial'>
                ...
                >
                  <div className='flex items-center space-x-4'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-gray-500 dark:text-gray-400'>
                        {formatDate(config.data_inicio)} -{' '}
                        {formatDate(config.data_fim)}
                      </p>
                      {config.horario_inicio && config.horario_fim && (
                        <p className='text-xs text-gray-400 dark:text-gray-500'>
                          {formatTime(config.horario_inicio)} -{' '}
                          {formatTime(config.horario_fim)}
                        </p>
                      )}
                    </div>
                  </div>
                  <div className='flex items-center space-x-2'>
                    <span
                      className={`px-2 py-1 text-xs rounded-full ${getTipoColor(config.tipo)}`}
                    >
