...
const DashboardFinanceiro: React.FC = () => {
  ...
  const loadMetricas = async () => {
    try {
      ...
      // Calcular métricas
      const receitaTotal =
        receitas?.reduce((acc: any, r: any) => acc + r.valor, 0) || 0;
      const receitaMes =
        receitas
          ?.filter((r: any) => new Date(r.created_at) >= inicioMes)
      ...
      const despesasTotal =
        despesas?.reduce((acc: any, d: any) => acc + d.valor, 0) || 0;
      const despesasMes =
        despesas
          ?.filter((d: any) => new Date(d.data_movimento) >= inicioMes)
      ...
      const valorContasVencidas =
        contasVencidas?.reduce((acc: any, c: any) => acc + c.valor, 0) || 0;

      const ticketMedio = receitas?.length ? receitaTotal / receitas.length : 0;
      const ticketMedioAnterior =
        receitas?.filter((r: any) => {
          const data = new Date(r.created_at);
          return data >= inicioMesAnterior && data <= fimMesAnterior;
        }).length || 0;
      const ticketMedioAnteriorValor =
        ticketMedioAnterior > 0 ? receitaAnterior / ticketMedioAnterior : 0;
  ...
  const loadDadosGrafico = async () => {
    try {
      ...
      const dataInicio = new Date(
        hoje.getTime() - diasAtras * 24 * 60 * 60 * 1000
      );
  ...
  return (
    <div className='min-h-screen bg-gray-50 dark:bg-gray-900 p-6'>
      <Helmet>
        ...
        <meta
          name='description'
          content='Visão geral das finanças da clínica'
        />
      </Helmet>
              ...
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <div className='flex items-center mt-2'>
                    ...
                    <span
                      className={`text-sm font-medium ml-1 ${getCrescimentoColor(metricas.crescimentoReceita)}`}
                    >
              ...
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <div className='flex items-center mt-2'>
                    ...
                    <span
                      className={`text-sm font-medium ml-1 ${getCrescimentoColor(metricas.crescimentoLucro)}`}
                    >
