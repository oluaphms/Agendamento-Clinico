...
const ContasReceber: React.FC = () => {
  ...
  const contasFiltradas = contas.filter(conta => {
    ...
    const matchesDataInicio = !filtros.data_vencimento_inicio || new Date(conta.data_vencimento) >= new Date(filtros.data_vencimento_inicio);
    const matchesDataFim = !filtros.data_vencimento_fim || new Date(conta.data_vencimento) <= new Date(filtros.data_vencimento_fim);
    const matchesBusca = !filtros.busca || 
      conta.paciente?.nome.toLowerCase().includes(filtros.busca.toLowerCase()) ||
  ...
  const contasVencidas = contas.filter(c => c.status === 'vencido' || (c.status === 'pendente' && isVencida(c.data_vencimento))).length;
  const valorTotal = contas.filter(c => c.status === 'pago').reduce((acc, c) => acc + c.valor, 0);
  const valorPendente = contas.filter(c => c.status === 'pendente').reduce((acc, c) => acc + c.valor, 0);
          ...
          {contasFiltradas.map((conta) => {
            ...
            return (
              <Card key={conta.id} className={`hover:shadow-lg transition-shadow ${
                vencida && conta.status === 'pendente' ? 'border-red-200 dark:border-red-800' : ''
              }`}>
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-400">
                        <div className="flex items-center space-x-1">
                          ...
                          <span>Vence em {formatDate(conta.data_vencimento)}</span>
                        </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900 dark:text-white">
                        {conta.agendamento?.data ? formatDate(conta.agendamento.data) : 'N/A'}
                      </p>
                      <p className="text-gray-500 dark:text-gray-400">
                  ...
                  {conta.observacoes && (
                    <div className="mt-4">
                      ...
                      <p className="text-gray-900 dark:text-white text-sm">{conta.observacoes}</p>
                    </div>
                  )}
                </CardContent>
              </Card>
