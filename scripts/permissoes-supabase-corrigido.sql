-- =====================================================
-- SCRIPT DE PERMISSÕES CORRIGIDO PARA SUPABASE
...
-- =====================================================
-- VERIFICAR TIPO DA COLUNA ID DA TABELA USUARIOS
-- =====================================================
...
DECLARE
    id_type TEXT;
BEGIN
    -- Verificar o tipo da coluna id da tabela usuarios
    ...
    IF id_type IS NULL THEN
        RAISE EXCEPTION 'Tabela usuarios não encontrada. Execute primeiro o schema principal.';
    END IF;
    
    RAISE NOTICE 'Tipo da coluna id da tabela usuarios: %', id_type;
    
    -- Se for INTEGER, vamos usar INTEGER para as referências
    IF id_type = 'integer' THEN
        RAISE NOTICE 'Usando INTEGER para as referências de chave estrangeira';
    ELSIF id_type = 'uuid' THEN
        RAISE NOTICE 'Usando UUID para as referências de chave estrangeira';
    ELSE
        RAISE EXCEPTION 'Tipo de ID não suportado: %', id_type;
    END IF;
END $$;
...
-- Tabela de relacionamento user-roles (com tipo dinâmico)
...
-- =====================================================
-- ADICIONAR CHAVES ESTRANGEIRAS DINAMICAMENTE
-- =====================================================
...
BEGIN
    ...
    -- Adicionar chave estrangeira baseada no tipo
    IF id_type = 'integer' THEN
        -- Se for INTEGER, converter UUID para INTEGER (não recomendado, mas funcional)
        ...
        RAISE NOTICE 'Chaves estrangeiras adicionadas com tipo INTEGER';
    ELSIF id_type = 'uuid' THEN
        ...
        RAISE NOTICE 'Chaves estrangeiras adicionadas com tipo UUID';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Erro ao adicionar chaves estrangeiras: %', SQLERRM;
        RAISE NOTICE 'Continuando sem chaves estrangeiras...';
END $$;
...
-- =====================================================
-- ATRIBUIR ROLES A USUÁRIOS EXISTENTES (SEM CHAVE ESTRANGEIRA)
-- =====================================================
...
AND NOT EXISTS (
    ...
    WHERE ur.user_id = u.id::text::uuid AND ur.role_id = r.id
)
LIMIT 1;
...
SELECT 
    u.id::text::uuid,
    r.id,
    (SELECT id::text::uuid FROM usuarios WHERE nivel_acesso = 'admin' LIMIT 1)
FROM usuarios u, roles r
...
BEGIN
    ...
    RAISE NOTICE 'NOTA: As chaves estrangeiras podem não ter sido criadas devido a incompatibilidade de tipos.';
    RAISE NOTICE 'O sistema funcionará normalmente, mas sem integridade referencial automática.';
END $$;
