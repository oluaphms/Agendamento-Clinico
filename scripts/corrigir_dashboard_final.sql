-- =====================================================
-- CORRIGIR DASHBOARD - VERSÃO FINAL SIMPLES
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN ROW_NUMBER() OVER (ORDER BY sc.quantidade DESC) = 1 THEN '#3B82F6'::VARCHAR(10)
            WHEN ROW_NUMBER() OVER (ORDER BY sc.quantidade DESC) = 2 THEN '#10B981'::VARCHAR(10)
            WHEN ROW_NUMBER() OVER (ORDER BY sc.quantidade DESC) = 3 THEN '#F59E0B'::VARCHAR(10)
            WHEN ROW_NUMBER() OVER (ORDER BY sc.quantidade DESC) = 4 THEN '#EF4444'::VARCHAR(10)
