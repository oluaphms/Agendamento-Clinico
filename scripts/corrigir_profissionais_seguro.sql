-- =====================================================
-- CORRIGIR PÁGINA PROFISSIONAIS - SCRIPT SEGURO
-- =====================================================
-- Este script corrige os problemas da página de Profissionais
-- de forma segura, verificando colunas existentes antes de usar
...
-- 3. CRIAR ÍNDICES PARA PERFORMANCE (APENAS COLUNAS EXISTENTES)
-- =====================================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profissionais' AND column_name = 'nome') THEN
        CREATE INDEX IF NOT EXISTS idx_profissionais_nome ON profissionais(nome);
    END IF;
    
    -- Índice para especialidade (sempre existe)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profissionais' AND column_name = 'especialidade') THEN
        CREATE INDEX IF NOT EXISTS idx_profissionais_especialidade ON profissionais(especialidade);
    END IF;
    
    -- Índice para cpf (sempre existe)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profissionais' AND column_name = 'cpf') THEN
        CREATE INDEX IF NOT EXISTS idx_profissionais_cpf ON profissionais(cpf);
    END IF;
    
    -- Índice para crm_cro (pode existir)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profissionais' AND column_name = 'crm_cro') THEN
        CREATE INDEX IF NOT EXISTS idx_profissionais_crm_cro ON profissionais(crm_cro);
    END IF;
    
    -- Índice para status (pode existir)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profissionais' AND column_name = 'status') THEN
        CREATE INDEX IF NOT EXISTS idx_profissionais_status ON profissionais(status);
    END IF;
    
    -- Índice para created_at (sempre existe)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profissionais' AND column_name = 'created_at') THEN
        CREATE INDEX IF NOT EXISTS idx_profissionais_created_at ON profissionais(created_at);
    END IF;
    
    -- Índice para email (pode existir)
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profissionais' AND column_name = 'email') THEN
