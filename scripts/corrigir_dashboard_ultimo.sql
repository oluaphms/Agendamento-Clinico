-- =====================================================
-- CORRIGIR DASHBOARD - VERSÃO ULTIMA SIMPLES
...
-- 5. CRIAR FUNÇÕES SIMPLES PARA GRÁFICOS (SEM PROBLEMAS DE TIPOS)
-- =====================================================
...
BEGIN
    ...
    SELECT 
        TO_CHAR(m.data_mes, 'Mon')::VARCHAR(10) as mes,
...
-- Função para distribuição de serviços (versão simplificada)
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(a.id) as valor,
        CASE 
            WHEN ROW_NUMBER() OVER (ORDER BY COUNT(a.id) DESC) = 1 THEN '#3B82F6'::VARCHAR(10)
            WHEN ROW_NUMBER() OVER (ORDER BY COUNT(a.id) DESC) = 2 THEN '#10B981'::VARCHAR(10)
            WHEN ROW_NUMBER() OVER (ORDER BY COUNT(a.id) DESC) = 3 THEN '#F59E0B'::VARCHAR(10)
            WHEN ROW_NUMBER() OVER (ORDER BY COUNT(a.id) DESC) = 4 THEN '#EF4444'::VARCHAR(10)
            ELSE '#8B5CF6'::VARCHAR(10)
        END as cor
    FROM servicos s
...
BEGIN
    ...
    SELECT 
        CASE EXTRACT(DOW FROM d.data_dia)
            WHEN 0 THEN 'Dom'::VARCHAR(10)
            WHEN 1 THEN 'Seg'::VARCHAR(10)
            WHEN 2 THEN 'Ter'::VARCHAR(10)
            WHEN 3 THEN 'Qua'::VARCHAR(10)
            WHEN 4 THEN 'Qui'::VARCHAR(10)
            WHEN 5 THEN 'Sex'::VARCHAR(10)
            WHEN 6 THEN 'Sáb'::VARCHAR(10)
        END as dia,
...
-- 6. TESTAR FUNCIONALIDADES (SEM UNION)
-- =====================================================
