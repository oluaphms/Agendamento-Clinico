-- SCRIPT PARA CRIAR SISTEMA DE PERMISSÕES COM UUID
...
-- Atribuir role de Desenvolvedor aos desenvolvedores
INSERT INTO user_roles (user_id, role_id, assigned_by)
SELECT 
    u.id,
    r.id,
    (SELECT id FROM usuarios WHERE nivel_acesso = 'admin' LIMIT 1)
FROM usuarios u, roles r
WHERE u.nivel_acesso = 'desenvolvedor'
...
BEGIN
    ...
    RAISE NOTICE 'Roles disponíveis: Administrador, Recepcionista, Profissional de Saúde, Desenvolvedor';
END $$;
